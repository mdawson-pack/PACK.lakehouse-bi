...
export type ExportQueryOptions = {
  ...
  csv?: { compression?: ExportCompression; header?: boolean; delim?: string; };
  json?: { array?: boolean; compression?: ExportCompression; };
  parquet?: { compression?: string; parquetVersion?: "v1" | "v2" | "V1" | "V2"; };
  xlsx?: { header?: boolean; sheet?: string; sheetRowLimit?: number; };
};
...
async function simulateLocalExport(sql: string, options: ExportQueryOptions) {
  console.info("Export requested in local preview", { sql, options });
  window.alert?.(`Export requested: ${options.filename ?? options.title}.${options.format}. Saved dives generate and download the real file.`);
}
...
class QueryObserver {
  private state: QueryObserverState = { status: "idle", data: undefined, error: undefined, hasHadData: false, lastData: undefined };
  ...
  async execute(connection: MDConnection, sql: string) {
    ...
    try {
      ...
      if (result.status === "error") { this.setState({ status: "error", error: result.err, data: undefined }); return; }
      const data = result.result.data.toRows();
      this.setState({ status: "success", data, error: undefined, hasHadData: true, lastData: data });
    } catch (err) {
      if (signal.aborted) return;
      this.setState({ status: "error", error: err instanceof Error ? err : new Error(String(err)), data: undefined });
    }
  }

  reset() { this.abortController?.abort(); this.abortController = null; this.setState({ status: "idle", data: undefined, error: undefined }); }
...
export function useSQLQuery<TData = readonly DuckDBRow[]>(sql: string, options?: UseSQLQueryOptions<TData>): UseSQLQueryResult<TData> {
  ...
  useEffect(() => {
    if (!enabled || connState.status !== "connected") { if (observer.getStatus() !== "idle") observer.reset(); return; }
    observer.execute(connState.connection, sql);
    return () => observer.cancel();
  }, [sql, enabled, connState, observer]);

  const refetch = useCallback(() => { if (connState.status === "connected" && enabled) observer.execute(connState.connection, sql); }, [observer, connState, enabled, sql]);
  const exportAs = useCallback(async (exportOptions: ExportQueryOptions) => { if (connState.status !== "connected") throw new Error("Not connected"); await simulateLocalExport(sql, exportOptions); }, [connState.status, sql]);
  ...
  const { data, isPlaceholderData } = useMemo(() => {
    ...
    if (!snap.hasHadData && options?.initialData !== undefined) return { data: options.initialData, isPlaceholderData: false };
    if (isLoading && options?.placeholderData !== undefined) {
      const ph = typeof options.placeholderData === "function" ? (options.placeholderData as (p: TData | undefined) => TData | undefined)(transformed) : options.placeholderData;
  ...
  return { data, isLoading, isSuccess: snap.status === "success", isError: snap.status === "error", isPlaceholderData, error: snap.error ?? null, refetch, exportAs, status: snap.status };
}
...
export function useConnectionStatus() {
  const { state } = useSDKContext();
  return { isConnected: state.status === "connected", isConnecting: state.status === "connecting", error: state.status === "error" ? state.error : null };
}
